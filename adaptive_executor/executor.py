...
class AdaptiveExecutor(ParslExecutor):
    ...
    def __process_queue(self) -> None:
        ...
        if cur_jobs  < self.max_jobs: # type: ignore
            sub_thread = Thread(target=self.__submit_slurm_job, args=(cluster,cores,), daemon=True)
            sub_thread.start()
        elif len(cluster) > 0:
            send_thread = Thread(target=self.__send_tasks, args=(cluster,False,), daemon=True)
    ...
    def __submit_slurm_job(self, cluster: list, max_workers: int) -> None:
        ...
        with self.lock:
            ...
            if len(self.current_jobs) == self.max_jobs:
                return
            launch_cmd = DEFAULT_LAUNCH_CMD.format(hostname = self.address, pull_port = self.pull_port, push_port = self.push_port, ack_port = self.ack_port, timeout = 30, max_workers=max_workers)
        ...
        time.sleep(5) # Sleep X seconds to await the slurm to process the job submission just to wait for slurm to process
    ...
    def shutdown(self) -> None:
        ...
        for t in tasks_copy:
            if tasks_copy[t]["status"] in ["sent", "queued"]:
                with self.lock:
                    self.future_tasks[t].set_exception(ValueError("Task failed unexpectedly"))
